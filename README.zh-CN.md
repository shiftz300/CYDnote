...
- 页面提供可选的 `gh-proxy` 镜像开关，并会在 `gh-proxy.org`、`edgeone.gh-proxy.org`、`cdn.gh-proxy.org`、`hk.gh-proxy.org` 之间回退下载固件 bin
...
- 页面会通过 `gh-proxy.org`、`edgeone.gh-proxy.org`、`cdn.gh-proxy.org`、`hk.gh-proxy.org` 回退代理固件下载 URL；若直连 GitHub API 失败，也会自动尝试这些代理及 `https://github.akams.cn/` 拉取 release 元数据
...
- 范围：`0x20-0x7F, 0x3000-0x303F, 0x3040-0x30FF, 0x4E00-0x9FAF, 0x0400-0x04FF, 0x00A0-0x00FF, 0x0370-0x03FF, 0x2000-0x206F, 0x2070-0x209F, 0x2100-0x214F, 0x2190-0x21FF, 0x2200-0x22FF, 0xFF00-0xFFEF`
- 字号：`14`
- 位深：`2-bit-per-pixel`
- 压缩：启用
...
- CJK 符号和标点：`0x3000-0x303F`（64，含 `——` / `…`）
- 全角形式与中文常见全角标点：`0xFF00-0xFFEF`（240）
- 希腊字母（物理/数学常用）：`0x0370-0x03FF`（144）
- 通用标点 + 上下标 + 字母类符号：`0x2000-0x214F`（336）
- 箭头与数学运算符：`0x2190-0x22FF`（368）
- Latin-1 补充（单位与常见符号）：`0x00A0-0x00FF`（96）
